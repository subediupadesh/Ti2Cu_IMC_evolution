...
[ICs]
    ...
    [eta3]
        variable = eta3
        type = FunctionIC
        function = 'if(y>200&y<=300 & x>100&x<=200, 1, 0)'
    []

    [eta4]
        variable = eta4
        type = FunctionIC
        function = 'if(y>200&y<=300  & x>300&x<=400, 1, 0)'
    []

    [eta5]
        variable = eta5
        type = FunctionIC
        function = 'if(y>100&y<=200  & x>200&x<=300, 1, 0)'
    []
    ...
    [eta1]
        ...
        function = 'if(y>300&y<=400 & x>200&x<=300, 0,
                    if(y>200&y<=300 & x>100&x<=200, 0,
                    if(y>200&y<=300 & x>300&x<=400, 0,
                    if(y>100&y<=200 & x>200&x<=300, 0, 1))))'
    []
    ...
    [c]
        ...
        function =  '0.033* if(y>300&y<=400 & x>200&x<=300, 1,
                            if(y>200&y<=300 & x>100&x<=200, 1,
                            if(y>200&y<=300 & x>300&x<=400, 1,
                            if(y>100&y<=200 & x>200&x<=300, 1, 0))))
...
[Materials]
    ...
    [scale] # m-->nm J-->eV s-->10ms v_mol-->m^3/mol (Cu-> 7.11e-6 m^3/mol; Ti->10.64e-6 m^3/mol)
    ...
    [constants]
        ...
        prop_values = '15.0  10.0e-9  1.5  2.0e-10   2.0e-8    2.0e-8      2.0e-8   8.31' #sigma -> J/m^2; delta -> meter; M_si unit-> m^5/Js 
    []
    ...
    [L1-2]
        ...
        function = 'factor_L*(0.5/0.2)*(M1+M2)*(mu/kappa)*(length_scale^3/(time_scale*energy_scale))' 
    []
